...
<link rel = "stylesheet" href = "<?php echo $styles_path . '/' . $style_page; ?> "">
...
<!-- 8. REPLACE the date shown below with PHP code that shows the current date in the format shown below. You will need to look for the correct letters to use in the date() function, using the reference page at php.net. The month below is the full textual representation of the month. The day is the day of the month *without* a leading zero. The year is the full numeric representation of the year. The minutes are the 12-hour format *without* a leading zero.  HINT: Refer to Part 1 for very similar code. You will need to change almost all of the letters, based on what you find on the date reference page at php.net . Part 1 will tell you how to find that reference page. There won't be any slashes in the date; they will be replaced by spaces and a comma. The part inside the parentheses after date() will be in this format: "X x, X x:x x" -->

<h3><?php echo date("F, j Y h:i a"); ?></h3>
