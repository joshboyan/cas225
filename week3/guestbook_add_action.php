...
/* guestbook_add_action-starter.php Revision 4 10-11-15 9:20pm
...
// TEST CODE -- comment out next 2 lines when code for the first page submits correctly. See Assignment 3 Part 2 for an explanation.
...
<!-- 4. COPY the line for the second link from your guestbook_add.php page. This line must use PHP for credit. -->
...
$name = str_replace("/","", $name); //remove forward slashes
$name = addslashes($name); // escape quote marks so they will work in SQL statements

$email = trim($_POST["email"]);
$email = stripslashes(strip_tags($email)); // strip html tags and back slashes
$email = str_replace("/","", $email); //remove forward slashes
$email = addslashes($email); // escape quote marks so they will work in SQL statements

$comment = trim($_POST["comment"]);
$comment = stripslashes(strip_tags($comment)); // strip html tags and back slashes
$comment = str_replace("/","", $comment); //remove forward slashes
$comment = addslashes($comment); // escape quote marks so they will work in SQL statements
...
echo "Do you like mail?";

if(isset($_POST["mail"])) {
    echo "<b> Thank you for signing up for ourlist! </b></br>";
}
else {
    echo "<b> No?! Boo! </b>";
}
