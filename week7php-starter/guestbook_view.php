...
/* 4. Create an SQL statement that selects all of the records from the guestbook table and orders the results
...
while($rows = $result->fetch(PDO::FETCH_ASSOC)) {
   ...
   /* 8. BETWEEN the echo "<tr>"; and echo "</tr>"; lines below, INSERT 4 rows of code* to display your data in
	 table cells. The first row will contain 'id', the second row will contain 'username', the third row
	 will contain 'comment', and the 4th row will contain 'datetime'. In each case you 
	 will be displaying an element of the the $rows[] array. On all 4 rows you 
	 will need to surround the data with <td> and </td> tags. 
	 HINT: See Step 3: Use Returned Data (if any), c in Part 1 for the code I would like you to use. 
      Be sure it is the longer version, which contains table tags. Use ONLY the code for the rows that appears between
      the echo "<tr>"; and echo "</tr>"; tags. You will need to change the field names from 'SongID', 'Song' and
      'DiscID' to the correct field names for the guestbook table. There should be only 4 lines of code. */
